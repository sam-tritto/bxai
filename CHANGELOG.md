...
### Added
- **ROPE (Region of Practical Equivalence) for `BayesianPermutation`**: Added `rope` parameter to `BayesianPermutation` supporting float thresholds (defining a symmetric `[-rope, rope]` interval) and custom boundaries `(rope_lower, rope_upper)`. This helps explicitly classify features with negligible effects as `Rejected` instead of leaving them `Tentative`, and supports shading and labeling the ROPE bounds in matplotlib diagnostic plots.
