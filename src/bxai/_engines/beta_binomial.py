...
class BetaBinomialTracker:
    ...
    def update(self, hits: np.ndarray, indices: np.ndarray | None = None) -> None:
        ...
        if indices is None:
            if len(hits) != self.n_features:
                raise ValueError(
                    f"Expected hits of length {self.n_features}, got {len(hits)}"
                )
            self.alpha += hits
            self.beta += 1.0 - hits
        else:
            ...
            if len(hits) != len(indices):
                raise ValueError(
                    f"Length of hits ({len(hits)}) must match length of indices ({len(indices)})"
                )
